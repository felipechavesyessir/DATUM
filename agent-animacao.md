# Agent Instructions - DATUM Animacao da Logotipo

Este arquivo define as instrucoes obrigatorias para qualquer agente que altere, revise ou recrie a animacao da logotipo DATUM no inicio do site.

## Regra Principal Obrigatoria

A animacao deve sempre seguir este comportamento:

1. O `D` da logotipo DATUM deve sair visualmente la de cima, vindo da area do header/topo.
...
4. No centro da tela, deve aparecer o restante da palavra, apenas `atum`, formando visualmente `Datum`.
...
```text
[D oficial da logotipo] + [atum]
```

Regras:

- O `D` vem da logotipo oficial vetorizada.
- O texto adicional deve ser somente `atum`.
- Nao escrever `Datum` inteiro como texto, porque o `D` ja e a logotipo.
...
- Antes de comecar a animacao da DATUM e durante a animacao, as curvas de nivel devem se movimentar de forma suave, como terrain lines.
- O `D` deve parecer sair da propria logotipo do header.

### Durante o scroll

- O `D` deve completar a composicao com `atum`.
...
- Logo no segundo movimento de scroll, a palavra `Datum` deve desaparecer com animacao.
...
As curvas de nivel do fundo devem ser baseadas em `assets/brand/datum-topographic-background-vector.svg`, ocupar a tela inteira no estado inicial e ser SVG real ou outro recurso animavel equivalente, nao apenas uma imagem estatica, para permitir movimento sutil dos tracos.
...
- Usar a palavra `Datum` inteira como texto comum.
...
2. Scroll inicial: o `D` sai da logo superior esquerda e se conecta ao restante `atum` no centro.
...
Observacao: se o arquivo com acento existir como `DATUM ANIMAÇÃO.md`, ele e a referencia detalhada da especificacao atual.
